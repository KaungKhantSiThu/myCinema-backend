...
public class AdminMovieController {
    ...
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "Search completed successfully (may return empty list if no results)",
                    content = @Content(schema = @Schema(implementation = ExternalMovieSearchResponse.class))
            ),
            @ApiResponse(
                    responseCode = "400",
                    description = "Invalid search parameters (query too short or invalid page number)"
            ),
            @ApiResponse(
                    responseCode = "401",
            ...
            @ApiResponse(
                    responseCode = "502",
                    description = "External API error - TMDb service unavailable or API key invalid"
            )
    })
    public ResponseEntity<List<ExternalMovieSearchResponse>> searchMovies(
            ...
            @io.swagger.v3.oas.annotations.Parameter(
                description = "Search query (movie title, actor name, or keyword)",
                example = "Inception",
                required = true
            )
            String query,
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(
                    responseCode = "400",
                    description = "Invalid request - external ID required"
            ),
            @ApiResponse(
                    responseCode = "401",
                    description = "Not authenticated - valid JWT token required"
            ...
            @ApiResponse(
                    responseCode = "404",
                    description = "Movie not found in external source with provided ID"
            ),
            @ApiResponse(
                    responseCode = "502",
                    description = "External API error - TMDb service unavailable"
            )
    })
